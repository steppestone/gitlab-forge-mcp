 ...
 */
export function mcpToolError(toolName: string, sourceFile: string, e: unknown): {
  ...
  return {
    content: [{
      type: "text",
      text: `MCP tool "${toolName}" failed: ${msg}\n\nPlease inform the user of this error so they can investigate and fix it in the gitlab MCP server source at mcp-servers/gitlab/src/tools/${sourceFile}`,
    }],
    isError: true,
  };
}
