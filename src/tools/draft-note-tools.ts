...
import { resolveProject, textResult, formatPositionRef, projectParam } from "./shared.js";
...
function formatDraftNote(d: GitLabDraftNote): string {
  ...
  if (d.position) {
    lines.push(`  [${formatPositionRef(d.position)}]`);
  }
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid }) => {
      try {
        ...
        if (drafts.length === 0) {
          return textResult(`MR !${mr_iid}: No draft notes pending.`);
        }
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid, note, commit_id, in_reply_to_discussion_id, resolve_discussion, position }) => {
      try {
        ...
        return textResult(`Queued draft on MR !${mr_iid}.\n${formatDraftNote(draft)}`);
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid, draft_note_id, note, resolve_discussion, position }) => {
      try {
        ...
        return textResult(`Updated draft on MR !${mr_iid}.\n${formatDraftNote(draft)}`);
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid, draft_note_id }) => {
      try {
        ...
        return textResult(`Deleted draft note ${draft_note_id} on MR !${mr_iid}.`);
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid, draft_note_id, confirm }) => {
      try {
        ...
        return textResult(`Published draft note ${draft_note_id} on MR !${mr_iid}.`);
      } catch (e) {
  ...
  server.registerTool(
    ...
    async ({ project, mr_iid, confirm }) => {
      try {
        ...
        return textResult(`Submitted review: published all draft notes on MR !${mr_iid}.`);
      } catch (e) {
