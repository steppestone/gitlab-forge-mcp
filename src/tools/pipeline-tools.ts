...
import { formatDate, numParam, mcpToolError, MAX_LOG_CHARS } from "../utils.js";
import { resolveProject, textResult, formatDuration, formatBytes, projectParam, pageParams } from "./shared.js";
  ...
  server.registerTool(
    ...
    async ({ project, job_id, tail }) => {
      try {
        ...
        const maxLength = MAX_LOG_CHARS;
        if (log.length > maxLength) {
          log = `[Log truncated, showing last ${maxLength} characters]\n...\n${log.slice(-maxLength)}`;
        }

        return textResult(`Job #${job_id} Log:\n\n${log}`);
      } catch (e) {
