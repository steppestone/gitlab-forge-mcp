...
import { formatDate, truncateText, numParam, mcpToolError } from "../utils.js";
import { resolveProject, formatMergeRequest, formatCommit } from "./shared.js";
    ...
    async ({ project, mr_iid }) => {
      try {
        ...
        for (const change of changes.changes) {
          ...
          lines.push(truncateText(change.diff, 3000));
          lines.push("");
        }
  ...
  server.registerTool(
    "gitlab_get_mr_pipelines",
    {
      ...
      inputSchema: {
        ...
        mr_iid: numParam().describe("MR IID"),
        page: numParam().optional().default(1).describe("Page"),
        per_page: numParam().optional().default(10).describe("Per page"),
      },
    },
    async ({ project, mr_iid, page, per_page }) => {
      try {
